...
struct TokenIterator: IteratorProtocol {
    ...
    let tokenizers = [
        ...
        try! Tokenizer(pattern: "\\d{0,}\\.?\\d{1,}") {
            var numberStr = NSNumber(value: Double.nan)

            if let numberStr2 = numberFormatter.number(from: $0) {
                numberStr = numberStr2
            } else {
                print("SPCAssessment.Tokens.swift, number tokenizer failed to format the string '\(numberStr)'.")
            }
            
            return .number(numberStr.doubleValue)
        },
        
        // Match all characters between delimiters.
        try! Tokenizer(pattern: "[^\u{8}\\s{}()/]+") { word in
